  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <img
            src={libro.portadaUrl || "/placeholder.svg"}
            alt={libro.titulo}
        ...
        <div>
          ...
          <img
            src={libro.autor?.fotoUrl || "/placeholder.svg"}
            alt={libro.autor?.nombre || "Autor desconocido"}
            className="w-48 h-48 rounded-full my-4 object-cover"
          />
          <h3 className="text-xl">{libro.autor?.nombre || "Desconocido"}</h3>
          <p>{libro.autor?.biografia || "No hay biografía disponible."}</p>
        </div>
      </div>
    </div>
  );
